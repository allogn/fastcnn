...
void ConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    for (int n = 0; n < num_; ++n) {
      ...
      std::ifstream f;
      std::ofstream log;
      //log.open("/media/alvis/Data/Term1/networksconv/log.out", ios::app);
      clock_t t;
      clock_t tn;
      
      if (M_ == 96 && K_ == 363 && N_ == 3025)
      {
            //log << "--- First layer forward_cpu: \n";
            //log << "g = " << group_ << ";\n";
            ...
            u = new Dtype[r*M_];//malloc(size(Dtype) * r * M_);
            s = new Dtype[r];//malloc(size(Dtype) * r * r);        
            v = new Dtype[r*K_];//malloc(size(Dtype) * r * K_);
            vs = new Dtype[r*N_];//malloc(size(Dtype) * r * M_);
            forTime = new Dtype[M_*N_];
            
            Dtype temp;
            
            f.open("/media/alvis/Data/Term1/networksconv/u80.in");
            for (int k = 0; k < M_; k++)
                for (int l = 0; l < r; l++)
                {
                    f >> temp;
            ...
            f.open("/media/alvis/Data/Term1/networksconv/s80.in");
            for (int k = 0; k < r; k++)
            {
                f >> temp;
                s[k] = temp; 
            ...
            f.open("/media/alvis/Data/Term1/networksconv/v80.in");
            for (int k = 0; k < r; k++)
                for (int l = 0; l < K_; l++)
                {
                    f >> temp;
...
//            }
            ...
            caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, r, N_, K_,
            (Dtype)1., v, col_data, (Dtype)0., vs);
            ...
            caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, N_, r,
            (Dtype)1., u, vs, (Dtype)0., top_data + (*top)[i]->offset(n));
            
            tn = clock() - tn;
            //log << "Time = " << ((double)tn)/CLOCKS_PER_SEC << " sec.\n";
...
//            t = clock() - t;
            ...
            //log << "Time for original function: " << ((double)t)/CLOCKS_PER_SEC << "\n";
